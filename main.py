...
class InterviewDialog:
    ...
    def __init__(self, api_key, resume_path, job_description_path, model, api_url=None):
        self.api_key = api_key
        self.api_url = (
            api_url or os.getenv("ANTHROPIC_API_URL") or "https://api.anthropic.com"
        )
        self.resume_path = resume_path
        self.job_description_path = job_description_path
        self.model = model

        self.agent = SoftwareInterviewAgent(
            self.api_key,
            self.resume_path,
    ...
    def run_interview(self):
        ...
        self.update_stats(
            len("Start the interview with the introduction and small talk stage."),
            len(response),
        )
        ...
        self.update_stats(
            len(
                "Provide closing remarks and explain the next steps in the hiring process."
            ),
            len(response),
        )
        self.display_stats()
    ...
    def save_interview(self):
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- Percentage of context used: {(self.stats['total'] / 200000) * 100:.2f}%\n"
            )
